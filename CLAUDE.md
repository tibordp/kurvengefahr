...
- `pen` — pen/layer id. **Stamped at concatenation** (`buildPageGeometry`) from the element's
  top-level `DocElement.pen` (NOT a geometry param — a pen change is a cheap re-place/re-emit, never
  a regenerate; mirrors `transform`). The optimizer is **per-pen**: each pen's strokes stay
  contiguous (→ minimal swaps) and pen groups are plotted in **palette order** (`profile.pens`
  sequence, passed to `optimize` as `pen_order`). Emit groups by pen in that same order and drops
  the profile's shared **`pause` macro** between groups (see profile). A locked chain is single-pen.
  A future *natively multi-colour* element type sets per-stroke pens in its generator and opts out of
...
**Fiducial** (`document.fiducial`, `Fiducial | null`) is the exception that proves the rule: a
page-space alignment point that makes **motion but no mark**, so it's a top-level document property
(not an element, not in the Stroke IR) — at most one, placed by the `fiducial` tool, dragged via
`FiducialLayer`. `emit` prepends a start-of-print move over it at a high clearance Z + an `M0` pause
(same pen→nozzle transform as strokes); the preview shows the travel to it. Persisted in the
snapshot (back-compat: absent → null).
...
- **Invalidation taxonomy:** text/params → regenerate that element; transform/**pen** → re-place
  only; feeds/Z/preamble/offset → re-emit only.
...
`bed, origin, feeds, penZ{up,down,dwell}, penOffset{x,y,z}, pens, preamble, postamble, pause`.
- **Pens** = the colour palette (`{id, name, color}[]`); `color` is display-only. Plot order =
  this list's order. Editable in the inspector (Machine tab); each element references a pen by id.
- **`pause`** = the shared operator-pause macro (templated `{message}`), reused for pen swaps
  ("Change to <pen>") and the fiducial ("Align medium…"). Only the *pause* is the macro; the
  positioning **moves** (clearance lift, fiducial travel) are emitted by `emit` (they need the
  pen→nozzle transform). Default `G4 P500` + `M0 {message}`. Empty = no pause.
...
React 18 + TS + Vite. Zustand stores: `document` (authoritative — elements + profile + selection + fiducial),
